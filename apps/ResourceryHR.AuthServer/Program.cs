...
public class Program
{
    public static async Task<int> Main(string[] args)
    {
        ...
        try
        {
            Log.Information("Starting ResourceryHR.AuthServer.");
            ...
            builder.AddNpgsqlDbContext<IdentityDbContext>(
                connectionName: ResourceryHRNames.IdentityServiceDb,
                configure => configure.DisableRetry = true
            );
            builder.AddNpgsqlDbContext<SaaSDbContext>(
                connectionName: ResourceryHRNames.SaaSDb,
                configure => configure.DisableRetry = true
            );

            builder.Host.AddAppSettingsSecretsJson().UseAutofac().UseSerilog();

            await builder.AddApplicationAsync<ResourceryHRAuthServerModule>();
        ...
        {
            Log.Fatal(ex, "ResourceryHR.AuthServer terminated unexpectedly!");
            return 1;
        }
        finally
        {
